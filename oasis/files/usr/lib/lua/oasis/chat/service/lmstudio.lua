...
local calling		= require("oasis.chat.function.calling.lmstudio")
...
lmstudio.new = function()
		...
		obj._get_first_message = function(_, chunk_json)
		...
		obj.convert_schema = function(self, user_msg)
			-- When role:tool is present, results are sent to AI; do not include tools
			...
			-- Function Calling Schema injection via calling module
			user_msg = calling.inject_schema(self, user_msg)

			-- Inject temperature/max_tokens only when generating a title (compat behavior)
		...
		obj.handle_tool_result = function(_, chat, speaker, msg)
		...
		obj.handle_tool_call = function(_, chat, speaker, msg)
		...
		obj.prepare_post_to_server = function(self, easy, callback, form, user_msg_json)
			...
			local api_key = tostring(self.cfg.api_key or "")
			if #api_key > 0 then
		...
		obj.handle_tool_output = function(self, tool_info, chat)
			...
			if tool_info_tbl then
				-- Insert assistant message with tool_calls first
				local tool_calls = {}
